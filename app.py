...
def employee_statistics():
    try:
        ...
        for record in attendance_records:
            print(f"   Date: {record.work_date}, Check-in: {record.check_in_time}, Check-out: {record.check_out_time}, Work Hours: {record.work_hours}")
        ...
        # ============== حساب الحضور والساعات لجميع الورديات في اليوم ==============
        
        # 8. حساب أيام الحضور الفعلية والتأخير مع دعم الورديات المتعددة
        ...
        # تجميع السجلات حسب التاريخ لحساب إجمالي ساعات العمل
        daily_records = {}
        
        for record in attendance_records:
            if record.work_date:
                if record.work_date not in daily_records:
                    daily_records[record.work_date] = []
                daily_records[record.work_date].append(record)
        ...
        for record_date, records in daily_records.items():
            ...
            for record in records_sorted:
                if record.work_hours:
                    daily_work_hours += record.work_hours
                    print(f"   📊 Adding work hours for {record_date}: {record.work_hours} hours (total for day: {daily_work_hours})")
...
def get_dashboard_data():
    try:
        ...
        # جلب جميع سجلات الحضور لليوم مرتبة حسب الوقت (تصاعدي للحصول على أول دخول)
        ...
        # جلب جميع الإجازات الساعية المعتمدة لليوم
...
def handle_attendance():
    try:
        ...
        if not existing_record:  # Check-in
            ...
            # ✅ التحقق مما إذا كان هذا هو أول دخول للموظف اليوم
            ...
            if len(all_records_today) == 1:
                # ✅ التحقق من وجود إجازة ساعية تغطي وقت الدخول الحالي
                active_hourly_leave_at_checkin = LeaveRequest.query.filter(
                    LeaveRequest.employee_id == employee_id,
                    LeaveRequest.status == 'approved',
                    LeaveRequest.type == 'hourly',
                    LeaveRequest.start_date == today,
                ...
                # ✅ إذا لم يكن هناك إجازة ساعية تغطي وقت الدخول، نقوم بحساب التأخير
                if not active_hourly_leave_at_checkin:
                    # ✅ التحقق مما إذا كان هناك تأخير سابق لهذا الموظف في نفس اليوم
...
def update_attendance_control_record(record_id):
    ...
    try:
        ...
        if 'check_in_time' in data and data['check_in_time']:
            # تحليل الوقت المستلم من المتصفح (بصيغة datetime-local)
            # الوقت المستلم هو بالتوقيت المحلي للمتصفح
            try:
                check_in_naive = datetime.fromisoformat(data['check_in_time'])
                # نعتبر أن الوقت المستلم هو بتوقيت سوريا
        ...
        if 'check_out_time' in data and data['check_out_time']:
            # تحليل الوقت المستلم من المتصفح
            try:
